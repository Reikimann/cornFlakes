-- statuscolumn, terminal, toggle, words?, zen
-- NOTE: Remember to remove the other plugins already implementing that functionality
return {
  "folke/snacks.nvim",
  priority = 1000,
  lazy = false,
  ...
  init = function()
    vim.api.nvim_create_autocmd("User", {
      pattern = "VeryLazy",
      callback = function()
        Snacks.toggle.zen():map("<leader>az")
      ...
      enabled = true,
        preset = {
          keys = {
            ...
            { icon = " ", key = "c", desc = "Config", action = ":lua Snacks.dashboard.pick('files', {cwd = '~/cornFlakes'})" },
